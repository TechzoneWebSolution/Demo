...
namespace Demo.Services.V1
{
    public class StandardServices : AbstractStandardServices
    {
        ...
        public override SuccessResult<AbstractStandard> InsertUpdateStandards(AbstractStandard abstractStandard, IEnumerable<HttpPostedFileBase> news_Json, IEnumerable<HttpPostedFileBase> Live_Json = null, IEnumerable<HttpPostedFileBase> Demo_Json = null)
        {
            ...
            if (camp.Code == 200)
            {
                ...
                if (Demo_Json != null)
                {
                    if (Demo_Json.ElementAt(0) != null && camp != null)
                    {
                        abstractDocuments.Path = "Standard/" + camp.Item.Id + "/";
                        abstractDocuments.AvatarFolder = HttpContext.Current.Server.MapPath("~/Standard/" + camp.Item.Id + "/");
                        var file = Demo_Json.ElementAt(0);
                        string imgName = string.Empty;
                        if (!Directory.Exists(abstractDocuments.AvatarFolder))
                        {
                            Directory.CreateDirectory(abstractDocuments.AvatarFolder);
                        ...
                        abstractStandard.Demo_json = abstractDocuments.DocumentPath;
                        abstractStandard.Id = camp.Item.Id;
                        campgn = this.abstractStandardDao.InsertUpdateStandards(abstractStandard);
                    }
                }
